        ...
        {documents.slice(0, 4).map((item) => (
          <div key={item.id} className="flex items-center justify-between py-3 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {item.fileUrl && (
                <a href={item.fileUrl} target="_blank" rel="noreferrer" className="h-8 w-8 rounded-md bg-[#f5f5f6] border border-slate-200 inline-flex items-center justify-center">
                  <Download size={14} />
                </a>
              )}
            </div>
