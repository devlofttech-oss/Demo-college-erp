...
export default function FeeCollectionModal({ assignments, initialAssignmentId = '', onClose, onSave }) {
      ...
      <form onSubmit={submit} className="w-full max-w-xl bg-white rounded-xl shadow-2xl border border-slate-200">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">Manual/offline payment record only.</p>
          </div>
        ...
        <div className="p-6 grid sm:grid-cols-2 gap-4">
          ...
          <label>
            ...
            <input type="number" value={form.amount} onChange={(event) => setForm((prev) => ({ ...prev, amount: event.target.value }))} placeholder={selected?.dueAmount || 0} className="w-full h-11 rounded-lg border border-slate-200 px-3 text-sm" />
          </label>
          <label>
